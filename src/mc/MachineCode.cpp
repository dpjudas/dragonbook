...
size_t MachineCodeHolder::relocate(void* dest) const
{
	memcpy(dest, code.data(), code.size());
	...
	{
		const auto& funcEntry = functionTable.find(entry.func)->second;
		...
		{
			value = (ptrdiff_t)d + funcEntry.pos;
		}
